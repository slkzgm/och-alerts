...
export async function detectAndStoreReveal(
  tokenIdStr: string
): Promise<boolean> {
  ...
  console.log(
    `[detectAndStoreReveal] Fetching metadata for token #${tokenId} at: ${metadataUrl}`
  );
  ...
  try {
    ...
    if (!isRevealedNow) {
      ...
      console.log(
        `[detectAndStoreReveal] Token #${tokenId} is still unrevealed.`
      );
      return false;
    }
    ...
    console.log(
      `[detectAndStoreReveal] Token #${tokenId} is revealed. DB updated. Document:`,
      updatedDoc
    );
    return true;
  } catch (err: any) {
    console.error(
      `[detectAndStoreReveal] Error fetching metadata for token #${tokenId}:`,
      err.message
    );
    return false;
  }
